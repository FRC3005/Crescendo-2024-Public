 ...
 */
public class SwerveDriveCarpetKinematics extends SwerveDriveKinematics {
  // implements Kinematics<SwerveDriveCarpetKinematics.SwerveDriveWheelStates,
  ...
  private final String[] kModuleKeyPrefixPose = {
    "Carpet/FL_Pose", "Carpet/FR_Pose", "Carpet/RL_Pose", "Carpet/RR_Pose"
  };
  private final String[] kModuleKeyPrefixMagnitude = {
   ...
   *     the robot.
   */
  public SwerveDriveCarpetKinematics(
      CarpetTransform carpetTransform, Translation2d... moduleTranslationsMeters) {
  ...
  /**
   * Set this before calling odometry updates to correctly calculate each module position.
   *
   * @param currentRobotPose
   */
  public void setRobotPose(Pose2d currentRobotPose) {
    m_robotPose = currentRobotPose;
  }
  ...
  public Twist2d toTwist2d(SwerveDriveWheelPositions start, SwerveDriveWheelPositions end) {
    ...
    for (int i = 0; i < start.positions.length; i++) {
      ...
      double magnitude = endModule.distanceMeters - startModule.distanceMeters;
      double direction = 0.0;
      if (Math.signum(magnitude) < 0) {
        direction = 180.0;
      }

      // Robot relative swerve module transform, to field relative
      Transform2d moduleTransform =
          new Transform2d(
              m_modules[i], endModule.angle.rotateBy(Rotation2d.fromDegrees(direction)));
      Pose2d modulePose = m_robotPose.plus(moduleTransform);

      double transformValue = m_carpetTransform.getTransform(modulePose);
      magnitude *= transformValue;

      if (Math.abs(magnitude) >= 1E-9) {
        SmartDashboard.putNumberArray(
            kModuleKeyPrefixPose[i], DataTypes.pose2dToDataDegrees(modulePose));
        SmartDashboard.putNumber(kModuleKeyPrefixMagnitude[i], transformValue);
      }

      newPositions[i] = new SwerveModulePosition(magnitude, endModule.angle);
    }
